...
export default function Home() {
  ...
  const [stats, setStats] = useState({ totalAssets: 0, utilization: 0, pendingRequests: 0, totalUsers: 0 });
  ...
  useEffect(() => {
    // Fetch packages regardless of auth (public)
    API.get('/public/packages')
      .catch(err => console.error("Failed to load packages", err));

    if (user) {
      // Logged In: Fetch Real Dynamic Stats
      API.get('/public/stats')
        .then(res => setStats(res.data))
        .catch(err => console.error("Failed to load stats", err));
    } else {
      // Logged Out: Show Random/Demo Stats
      setStats({
        totalAssets: 124,
        utilization: 98,
        pendingRequests: 12,
        totalUsers: 500,
        recentAssignments: [
          { icon: "💻", assetName: "MacBook Pro M2", color: "bg-blue-100 text-blue-600" },
          { icon: "⌨️", assetName: "Logitech MX Master", color: "bg-orange-100 text-orange-600" },
          { icon: "🖱️", assetName: "Magic Trackpad", color: "bg-pink-100 text-pink-600" }
        ]
      });
    }
  }, [user]);
                  ...
                  <div className="flex gap-4 mb-6">
                    ...
                    <div className="w-1/3 h-24 rounded-2xl bg-emerald-50 border border-emerald-100 p-4">
                      <div className="text-emerald-600 font-bold text-2xl">
                        <CountUp to={stats.utilization} suffix="%" />
                      </div>
                    ...
                    <div className="w-1/3 h-24 rounded-2xl bg-purple-50 border border-purple-100 p-4">
                      <div className="text-purple-600 font-bold text-2xl">
                        <CountUp to={stats.pendingRequests} />
                      </div>
                      <div className="text-xs text-purple-400 uppercase mt-1">Pending</div>
                    </div>
                  </div>
                  <div className="space-y-3">
                    {(stats.recentAssignments && stats.recentAssignments.length > 0 ? stats.recentAssignments : [
                      { icon: "💻", assetName: "MacBook Pro", color: "bg-blue-100 text-blue-600" },
                      { icon: "⌨️", assetName: "Logitech MX", color: "bg-orange-100 text-orange-600" },
                      { icon: "🖱️", assetName: "Magic Mouse", color: "bg-pink-100 text-pink-600" }
                    ]).map((item, i) => {
                      // Helper to determine icon/color if coming from DB (where icon/color might be missing)
                      let icon = item.icon;
                      let color = item.color;

                      if (!icon) {
                        const name = (item.assetName || "").toLowerCase();
                        if (name.includes("mac") || name.includes("laptop")) { icon = "💻"; color = "bg-blue-100 text-blue-600"; }
                        else if (name.includes("key")) { icon = "⌨️"; color = "bg-orange-100 text-orange-600"; }
                        else if (name.includes("mouse")) { icon = "🖱️"; color = "bg-pink-100 text-pink-600"; }
                        else if (name.includes("monitor") || name.includes("screen")) { icon = "🖥️"; color = "bg-purple-100 text-purple-600"; }
                        else { icon = "📦"; color = "bg-gray-100 text-gray-600"; }
                      }

                      return (
                        <div key={i} className="w-full h-14 rounded-xl bg-white shadow-sm border border-slate-100 flex items-center px-4 gap-4">
                          <div className={`w-8 h-8 rounded-lg ${color || "bg-indigo-100 text-indigo-600"} flex items-center justify-center text-sm`}>{icon}</div>
                          <div className="flex-1">
                            <div className="h-2 w-24 bg-slate-100 rounded mb-1"></div>
                            <div className="text-[10px] font-bold text-slate-500 leading-none">{item.assetName}</div>
                          </div>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto bg-gradient-to-r from-indigo-600 to-purple-600 rounded-[3rem] p-12 md:p-20 text-white shadow-2xl relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <Link to="/register/hr" className="btn bg-red-500 text-indigo-600 btn-lg rounded-full px-12 h-16 text-lg hover:bg-indigo-50 border-0 shadow-xl ">
                Get Started Free
              </Link>
            </div>
          </div>
